...
export class Backtester {
  constructor({ provider, portfolio, strategies, symbols }) {
  ...
  async run({ from, to, interval = "day", csvFile = "trade_history.csv" }) {
    ...
    for (const symbol of this.symbols) {
      ...
      for (const bar of historical) {
        ...
        if (signal === "BUY") {
          ...
          this.trades.push({ date: bar.t, symbol, action: "BUY", price, quantity: 1, pnl: 0 });
        }
        if (signal === "SELL") {
          // calculate P/L
          ...
          this.trades.push({ date: bar.t, symbol, action: "SELL", price, quantity: 1, pnl: tradePnl });
        }

        // track portfolio value for drawdown
        const totalValue = this.portfolio.status(latestPrices).totalValue;
