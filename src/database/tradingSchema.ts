...
export class TradingDatabase {
  ...
  static async getActiveTradingSessions(userId: number): Promise<TradingSession[]> {
    return new Promise((resolve, reject) => {
      db.all(
        'SELECT * FROM trading_sessions WHERE user_id = $1 AND status = $2 ORDER BY created_at DESC',
        [userId, 'ACTIVE'],
        (err: any, rows: any[]) => {
          if (err) {
            reject(err);
  ...
  static async countActiveTradingSessions(userId: number): Promise<number> {
    return new Promise((resolve, reject) => {
      db.get(
        'SELECT COUNT(*) as count FROM trading_sessions WHERE user_id = $1 AND status = $2',
        [userId, 'ACTIVE'],
        (err: any, row: any) => {
          if (err) {
            reject(err);
          } else {
            resolve(typeof row?.count === 'number' ? row.count : parseInt(row?.count, 10) || 0);
          }
        }
      );
    });
